...
import {
  ResultsByChapter,
  IResultsByChapter,
} from "../schemas/resultsByChapterSchema";
import {
  ResultsByQuestion,
  IResultsByQuestion,
} from "../schemas/resultByQuestionSchema";
import { Chapter, IChapter } from "../schemas/chapterSchema";
...
import { ChapterTime, SubjectTime } from "../schemas/timeTracker";
import TotalQuestionDone from "../schemas/totalQuestionDoneScheme";
import TotalCorrectQuestions from "../schemas/totoalCorrectQuestions";
...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      let quiz: any = await Quiz.findOne({
        user_ref: userIdObj,
...
router.get("/:userId/total-time-spent", async (req: Request, res: Response) => {
  const userId = req.params.userId;
  ...
  try {
    ...
    const timeSpentByMode: { [key: string]: number } = {};
    const timeRef: { date: string; total_time_spent: string }[] = [];

    modes.forEach((mode) => {
      timeSpentByMode[mode] = 0;
    });

    const timeSpentByDate: { [key: string]: number } = {};

    if (user.quizzes_ref) {
      user.quizzes_ref.forEach((quiz: any) => {
    ...
    const formattedTotalTimeSpent = formatSecondsToHHMMSS(
...
router.get(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      const quizzesGroupedByDate = await Quiz.aggregate([
        {
          $match: {
      ...
      const results = await ResultsByQuestion.aggregate([
        {
          $lookup: {
      ...
      const stats = quizzesGroupedByDate.map((quizGroup) => {
        const resultForDate = results.find(
          (result) => result.date === quizGroup._id
        );
        return {
          date: quizGroup._id,
      ...
      const totalQuestionsDone = quizzesGroupedByDate.reduce(
        (sum, quizGroup) => sum + quizGroup.totalQuestions,
        0
      );
      const totalCorrectAnswers = stats.reduce(
...
router.get(
  "/:userId/correct/questions-per-date",
  async (req: Request, res: Response) => {
    const { userId } = req.params;
    try {
      const userObjectId = new Types.ObjectId(userId);
      ...
      const correctAnswers = await TotalCorrectQuestions.find({
        user_ref: userObjectId,
      });

      // Group correct answers by date and mode
      const correctAnswersByDate: any = correctAnswers.reduce(
        (acc: any, entry) => {
      ...
      // Calculate the total questions done and correct answers
      const totalQuestionsDone = stats.reduce(
        (sum, stat) => sum + stat.questionsDone,
        0
      );
      const totalCorrectAnswers = stats.reduce(
        (sum, stat) => sum + stat.correctAnswers,
...
router.get(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      const questionsGroupedByDate = await TotalQuestionDone.aggregate([
        {
          $match: {
            user_ref: userObjectId,
          },
      ...
      const totalQuestionsDone = questionsGroupedByDate.reduce(
        (sum, questionGroup) => sum + questionGroup.totalQuestions,
        0
      );
...
router.get(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      const timeSpentPerDate: { [key: string]: number } = {};
      ...
      const formattedResult = Object.entries(timeSpentPerDate).map(
        ([date, seconds]) => {
...
router.get("/:userId/total_questions", async (req: Request, res: Response) => {
  const userId = req.params.userId;
  try {
    ...
    res.json({
      totalQuestions:
        totalQuestions.length > 0 ? totalQuestions[0].totalQuestions : 0,
    });
  } catch (error) {
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const allQuestions = await Question.find({
        chapter_ref: { $in: allChapters.map((chapter: any) => chapter._id) },
      }).lean();

      // Fetch all chapter progress for the user, grouped by chapter and user
      const chapterProgresses = await UserChapterProgress.aggregate([
        {
          $match: {
            user_ref: userObjectId,
            chapter_ref: {
              $in: allChapters.map((chapter: any) => chapter._id),
            },
          },
        },
        {
      ...
      const fullResult = allSubjects.map((subject: any) => {
        // Find all chapters related to this subject
        const subjectChapters = allChapters.filter((chapter: any) =>
          chapter.subject_ref.equals(subject._id)
        );
        ...
        const chapters = subjectChapters.map((chapter: any) => {
          ...
          const chapterQuestions = allQuestions.filter((question: any) =>
            question.chapter_ref.equals(chapter._id)
          );

          // Get the progress for this chapter for the user
          const chapterProgress = chapterProgresses.find((progress: any) =>
            progress._id.chapter_ref.equals(chapter._id)
          );
        ...
        const totalQuestionCount = chapters.reduce(
          (acc: number, chapter: any) => acc + chapter.questionCount,
          0
        );
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const userChapterProgresses = await UserChapterProgress.find({
        user_ref: userId,
      }).exec();

      // Aggregate the total questions done by mode
      const questionsByMode: Record<string, number> = {};
...
router.get(
  "/:userId/:subjectId/subject-progress",
  async (req: Request, res: Response) => {
    try {
      const { userId, subjectId } = req.params;
      const userObjectId = new mongoose.Types.ObjectId(userId);
      const subjectObjectId = new mongoose.Types.ObjectId(subjectId);
      const result = await UserSubjectProgress.find({
        user_ref: userObjectId,
...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      const progress = await UserSubjectProgress.findOneAndUpdate(
        {
          user_ref: userObjectId,
          subject_ref: subjectObjectId,
        },
...
router.get(
  "/:userId/:chapterId/chapter-progress",
  async (req: Request, res: Response) => {
    try {
      const { userId, chapterId } = req.params;
      const userObjectId = new mongoose.Types.ObjectId(userId);
      const chapterObjectId = new mongoose.Types.ObjectId(chapterId);
      const result = await UserChapterProgress.find({
        user_ref: userObjectId,
...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      // Use findOneAndUpdate with upsert: true to either update or create a new document
      const progress = await UserChapterProgress.findOneAndUpdate(
        {
          user_ref: userObjectId,
          chapter_ref: chapterObjectId,
          mode,
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const totalCorrectQuestions = await TotalCorrectQuestions.find({
        user_ref: userId,
        mode: { $in: ["learn", "exam", "random"] },
      }).exec();
...
const getSubjectName = async (subjectId: string): Promise<string> => {
...
function parseTimeStringToSeconds(timeStr: any) {
...
function formatSecondsToTimeString(totalSeconds: any) {
...
router.post("/:userId/:chapterId/chapter-time-spent", async (req, res) => {
  const { userId, chapterId } = req.params;
  const { time_spent, mode } = req.body;

  try {
    const existingRecord = await ChapterTime.findOne({
      user_ref: userId,
    ...
    if (existingRecord) {
      ...
      const existingTimeInSeconds = parseTimeStringToSeconds(
        existingRecord.time_spent
      );
      ...
      const newChapterTime = new ChapterTime({
        chapter_ref: chapterId,
        user_ref: userId,
...
router.get("/:userId/:chapterId/get-chap-time", async (req, res) => {
  const { userId, chapterId } = req.params;

  try {
    // Query the database for the specific user and chapter
    const chapterTime = await ChapterTime.findOne({
      user_ref: userId,
      chapter_ref: chapterId,
    });
...
router.post("/:userId/:subjectId/subject-time-spent", async (req, res) => {
  const { userId, subjectId } = req.params;
  const { time_spent, mode } = req.body;

  try {
    const existingRecord = await SubjectTime.findOne({
      user_ref: userId,
    ...
    if (existingRecord) {
      ...
      const newSubjectTime = new SubjectTime({
        subject_ref: subjectId,
        user_ref: userId,
...
router.get("/:userId/:subjectId/get-sub-time", async (req, res) => {
  const { userId, subjectId } = req.params;

  try {
    // Query the database for the specific user and chapter
    const chapterTime = await SubjectTime.findOne({
      user_ref: userId,
      subject_ref: subjectId,
    });
...
// *********
router.get(
  "/:userId/time-per-subject-and-chapter",
  async (req: Request, res: Response) => {
    const userId = req.params.userId;
    try {
      // Fetch chapter times for the user and populate Chapter and Subject data
      const chapterTimes = await ChapterTime.find({
        user_ref: userId,
      }).populate({
        path: "chapter_ref",
        populate: {
      ...
      const chapterTimeMap: Record<string, string> = {}; // Storing time as HH:MM:SS
      const subjectTimeMap: Record<
        string,
        { total: string; chapters: Record<string, string> }
      > = {};
      ...
      chapterTimes.forEach((chapterTime) => {
        const chapter = chapterTime.chapter_ref as any;
        if (chapter && chapter.subject_ref) {
          const chapterName = chapter.chapter_name;
          chapterTimeMap[chapterName] = chapterTime.time_spent;
        } else {
          console.warn(
            `Warning: Missing chapter or subject reference for ChapterTime ID: ${chapterTime._id}`
          );
        }
      });
      ...
      subjectTimes.forEach((subjectTime) => {
        const subject = subjectTime.subject_ref as any;
        if (subject) {
          ...
          console.warn(
            `Warning: Missing subject reference for SubjectTime ID: ${subjectTime._id}`
          );
        }
      });
      ...
      // Format the response
      const totalTimePerChapter = Object.entries(chapterTimeMap).map(
        ([name, total]) => ({
          name,
      ...
      const totalTimePerSubject = Object.entries(subjectTimeMap).map(
        ([name, { total, chapters }]) => ({
          name,
          total: convertTimeToSeconds(total), // Convert to seconds
          chapters: Object.entries(chapters).map(
            ([chapterName, chapterTotal]) => ({
              name: chapterName,
...
router.get(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      const resultsGroupedByDate = await ResultsByQuestion.aggregate([
        {
          $lookup: {
            from: "resultsbychapters",
            localField: "results_by_chapter_ref",
      ...
      const stats = quizzesGroupedByDate.map((quizGroup) => {
        const resultForDate = resultsGroupedByDate.find(
          (result) => result._id.date === quizGroup._id
        );
        return {
          date: quizGroup._id,
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      // Fetch all quizzes associated with the user
      const quizzes = await Quiz.find({ user_ref: userObjectId }).exec();
      ...
      const chapterIds = resultsByChapters.map(
        (chapter) => chapter.chapter_ref
      );

      // Fetch available chapters using the chapter IDs
      const availableChapters = await Chapter.find({
        _id: { $in: chapterIds },
        is_available: true,
      }).exec();

      const availableChapterIds = availableChapters.map((chapter: any) =>
        chapter._id.toString()
      );

      // Filter resultsByChapters for only available chapters
      const filteredResultsByChapters = resultsByChapters.filter((chapter) =>
        availableChapterIds.includes(chapter.chapter_ref.toString())
      );

      const resultsByQuestionIds = filteredResultsByChapters.flatMap(
        (chapter) => chapter.results_by_question_ref
      );
      ...
      const resultsByQuestionMap = new Map<string, any>();
      const chapterExamModeMap = new Map<string, boolean>(); // To track if a chapter has exam mode quiz

      resultsByQuestions.forEach((result: any) => {
        const resultByChapter: any = filteredResultsByChapters.find(
          (chapter: any) =>
            chapter._id.toString() === result.results_by_chapter_ref.toString()
        );
        ...
        const isExamQuiz = quizzes.some(
          (quiz: any) =>
            quiz.is_exam_mode === true &&
            quiz.results_by_chapter_ref.some(
              (chapterRef: any) =>
                chapterRef.toString() === resultByChapter._id.toString()
            )
        );
      ...
      resultsByQuestionMap.forEach((entry, questionId) => {
        ...
        if (otherModeResults.length > 0) {
          otherModeResults.forEach((result: any) => {
            // console.log("Result", {result})
            if (!hasExamMode) {
              if (result.is_correct) {
...
router.post("/:userId/resetResults", async (req: Request, res: Response) => {
  const { userId } = req.params;
  ...
  try {
    if (!userId || !Array.isArray(chapterIds) || chapterIds.length === 0) {
      return res.status(400).json({
        message:
          "Invalid input. Provide a valid userId and an array of chapterIds.",
      });
    }
    const userIdObj = new mongoose.Types.ObjectId(userId);
    const chapterIdObjs = chapterIds.map(
      (id) => new mongoose.Types.ObjectId(id)
    );
    const quizzes = await Quiz.find({ user_ref: userIdObj }).distinct("_id");
    if (quizzes.length === 0) {
      return res
        .status(404)
        .json({ message: "No quizzes found for the given user." });
    }
    const resultsByChapterIds = await ResultsByChapter.find({
    ...
    if (resultsByChapterIds.length === 0) {
      return res.status(404).json({
        message:
          "No results by chapter found for the given chapters and quizzes.",
      });
    }
    await ResultsByQuestion.updateMany(
      {
    ...
    await ResultsByChapter.updateMany(
      { _id: { $in: resultsByChapterIds } },
      { $set: { questions_done: "0%" } }
    );
    await ResultsByChapter.find({
      _id: { $in: resultsByChapterIds },
    });
    res
      .status(200)
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      const resultsByChapters = await ResultsByChapter.find({
        ...
        quiz_ref: {
          $in: await Quiz.find({ user_ref: userId }).distinct("_id"),
        },
      }).populate(
        "results_by_question_ref",
        "question_ref is_correct is_not_correct done_by_mode answer_by_learn_mode is_learn_correct_answers is_learn_incorrect_answers to_fill_user_answer"
      );

      // Fetch the ResultsByQuestion documents for the fetched ResultsByChapter documents
      const results = await ResultsByQuestion.find({
        results_by_chapter_ref: {
          $in: resultsByChapters.map((chapter) => chapter._id),
        },
      }).populate(
        "question_ref",
        "q_latex_content q_latex_explanation q_answertype_options"
      );

      // Fetch the Chapter documents for the given chapter IDs
      const chapters = await Chapter.find({ _id: { $in: chapterIdsArray } });
      // console.log("Get user results: ", results);
      // Create a map of the results
      const resultMap = results.map((result: any) => ({
...
router.get(
  "/:userId/questions-done-percentage",
  async (req: Request, res: Response) => {
    const { userId } = req.params;
    ...
    try {
                    ...
                    $and: [
                      { $eq: ["$chapter_ref", "$$chapterId"] },
                      {
                        $in: [
                          "$quiz_ref",
                          await Quiz.find({
                            user_ref: new mongoose.Types.ObjectId(userId),
                          }).distinct("_id"),
                        ],
                      },
                    ],
      ...
      console.error(
        "Error calculating average percentage of questions done by subject:",
        error
      );
...
router.get(
  "/:userId/correct-answers-by-subject-and-chapter",
  async (req: Request, res: Response) => {
    try {
      const { userId } = req.params;
      const userObjectId = new mongoose.Types.ObjectId(userId);
      ...
      const user = await User.findById(
        userObjectId,
        "total_question_done university_ref"
      ).exec();
      if (!user) {
        return res.status(404).json({ message: "User not found" });
      }
      ...
      const quizzes = await Quiz.find({ user_ref: userObjectId }).lean();

      // Flatten the nested array of results_by_chapter_ref before filtering and querying
      const validResultsByChapterRefs = quizzes
        .flatMap((q: any) => q.results_by_chapter_ref) // Flatten the array
        .filter((ref: any) => mongoose.Types.ObjectId.isValid(ref));

      // Proceed with querying using the valid ObjectIds
      const resultsByChapters = await ResultsByChapter.find({
        _id: {
          $in: validResultsByChapterRefs.map(
            (ref: any) => new mongoose.Types.ObjectId(ref)
          ),
        },
      }).lean();

      const validResultsByQuestionRefs = resultsByChapters
        .flatMap((rc: any) => rc.results_by_question_ref) // Flatten again
        .filter((ref: any) => mongoose.Types.ObjectId.isValid(ref));

      const resultsByQuestions = await ResultsByQuestion.find({
        _id: {
          $in: validResultsByQuestionRefs.map(
            (ref: any) => new mongoose.Types.ObjectId(ref)
          ),
        },
      }).lean();
      ...
      const chapters = await Chapter.find({
        _id: { $in: resultsByChapters.map((rc: any) => rc.chapter_ref) },
      ...
      const subjectProgress = await UserSubjectProgress.find({
        user_ref: userObjectId,
        subject_ref: { $in: subjects.map((s: any) => s._id) },
      }).lean();

      // Group data manually
      const groupedData: any = {};

      resultsByChapters.forEach((resultsByChapter: any) => {
        const chapter: any = chapters.find((ch: any) =>
          ch._id.equals(resultsByChapter.chapter_ref)
        );
        const subject: any = subjects.find((sub: any) =>
          sub._id.equals(chapter.subject_ref)
        );
        ...
        resultsByChapter.results_by_question_ref.forEach((questionId: any) => {
          const question: any = resultsByQuestions.find((q: any) =>
          ...
          if (
            typeof question.is_exam_correct_answers === "number" &&
          ...
          const notAnsweredYet =
            question.exam_not_answered_yet !== false
      ...
      const chapterProgress = await UserChapterProgress.find({
        user_ref: userObjectId,
      }).lean();

      // Combine grouped data with all subjects to ensure every subject is included
      const finalResult = subjects.map((subject: any) => {
        ...
        const subjectChapters = allChapters
          .filter((ch: any) => ch.subject_ref.equals(subject._id))
          .map((chapter: any) => {
            ...
            const progressForChapter = chapterProgress.filter((cp: any) =>
              cp.chapter_ref.equals(chapter._id)
            );

            // Sum the total_questions_done for this chapter across all modes
            const totalQuestionsDone = progressForChapter.reduce(
              (sum: number, cp: any) => sum + (cp.total_questions_done || 0),
              0
            );
            return {
              ...chapterData,
        ...
        const subjectProgressForUser = subjectProgress.find((sp: any) =>
          sp.subject_ref.equals(subject._id)
        );

        // console.log("subjectProgressForUser", {subjectProgressForUser})
        const totalQuestionsDoneForSubject =
          subjectProgressForUser?.total_questions_done || 0;
      ...
      console.error(
        "Server error while getting the correct answer by subject and chapter:",
        error
      );
      res.status(500).json({ message: "Server Error" });
    }
...
router.get(
  "/questions-by-chapter/:userId/:chapterId",
  async (req: Request, res: Response) => {
    const { userId, chapterId } = req.params;

    try {
      // Find the quiz related to the user and chapter
      const userQuizzes = await Quiz.find({ user_ref: userId }).distinct("_id");

      // Find the results by chapter related to the user's quizzes
      const resultsByChapter = await ResultsByChapter.findOne({
        chapter_ref: chapterId,
...
router.get(
  "/:userId/exam-results/:chapterIds",
  async (req: Request, res: Response) => {
    ...
    try {
        ...
        .populate({
          ...
          populate: {
            ...
            populate: {
              path: "question_ref",
              select:
                "q_answertype_options q_answertype_options_has_multiple_good_answers q_latex_explanation q_latex_explanation_ChatGPT q_image_url q_latex_content q_answertype_tofill",
            },
          },
        })
        .populate({
      ...
      quizzes.forEach((quiz: any) => {
        quiz.chapters_ref.forEach((chapter: any) => {
          ...
          quiz.results_by_chapter_ref.forEach((resultsByChapter: any) => {
            if (resultsByChapter.chapter_ref.toString() === chapterId) {
              resultsByChapter.results_by_question_ref.forEach(
                (result: any) => {
                  const question = result.question_ref;

                  // Filter out options with empty latex_content and image_url
                  const filteredOptions = question.q_answertype_options.filter(
                    (option: any) => option.latex_content || option.image_url
                  );
                  ...
                  subjectsMap[subjectId].chapters[chapterId].results.push({
                    ...
                    q_answertype_options_has_multiple_good_answers:
                      question.q_answertype_options_has_multiple_good_answers,
                    q_latex_explanation: question.q_latex_explanation,
                    q_latex_explanation_ChatGPT:
                      question.q_latex_explanation_ChatGPT,
...
router.put("/:resultId/update-exam-results", async (req, res) => {
  const { resultId } = req.params;
  const { is_exam_correct_answers, is_exam_incorrect_answers } = req.body;

  try {
    // Find the result by ID
    const result = await ResultsByQuestion.findById(resultId);
...
router.post("/increment-questions", async (req: Request, res: Response) => {
  ...
  try {
    ...
    let totalQuestionRecord = await TotalQuestionDone.findOne({
      user_ref: user_id,
      mode: mode,
    ...
    if (!totalQuestionRecord) {
      totalQuestionRecord = new TotalQuestionDone({
        user_ref: user_id,
        mode: mode,
...
router.get(
  "/increment-questions/correct/get/:user_id/:chapter_id/:mode",
  async (req: Request, res: Response) => {
    const { user_id, chapter_id, mode } = req.params;
    try {
      const result = await TotalCorrectQuestions.findOne({
        user_ref: user_id,
        mode: mode,
...
router.post(
  "/increment-questions/correct",
  async (req: Request, res: Response) => {
    const { user_id, mode, correct_answers_count, chapter_id, question_id } =
      req.body;
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      for (const chapterId of chapterIds) {
        ...
        const objectIdChapterId = new mongoose.Types.ObjectId(chapterId);

        // Update the TotalCorrectQuestions collection for each chapter
        const result = await TotalCorrectQuestions.updateOne(
          {
